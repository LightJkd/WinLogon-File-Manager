    ...
    loop {
        ...
        match choice {
            "1" => {
                ...
                match add_to_winlogon(file_path) {
            ...
            "2" => {
                ...
                match delete_from_winlogon(file_path) {
...
fn add_to_winlogon(file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn delete_from_winlogon(file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    let hklm = RegKey::predef(HKEY_LOCAL_MACHINE);
    let winlogon_key = hklm.open_subkey_with_flags("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon", KEY_ALL_ACCESS)?;
